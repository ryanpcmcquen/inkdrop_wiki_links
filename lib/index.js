...
module.exports = {
  ...
  activate() {
    if (_inkdrop.markdownRenderer) {
      _inkdrop.markdownRenderer.remarkPlugins.push(_wiki_link_plugin.default); // markdownRenderer.remarkReactComponents.wikiLink = WikiLink;
  ...
  deactivate() {
    if (_inkdrop.markdownRenderer) {
      _inkdrop.markdownRenderer.remarkPlugins = _inkdrop.markdownRenderer.remarkPlugins.filter(plugin => plugin !== _wiki_link_plugin.default); // if (remarkReactComponents.wikiLink === WikiLink) {
